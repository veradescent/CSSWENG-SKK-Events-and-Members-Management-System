...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res.status(400).render('login', { title: 'Login', error: 'Email and password required', layout: false });
    }

    const user = await User.findOne({ email }).lean().exec();
    if (!user) {
      return res.status(401).render('login', { title: 'Login', error: 'Invalid credentials', layout: false });
    }
    ...
    return res.status(500).render('login', { title: 'Login', error: 'Server error', layout: false });
  }
});
