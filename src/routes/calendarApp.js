document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderMonthView() {
    ...
    for (let r = 0; r < 5; r++) {
      ...
      for (let c = 0; c < 7; c++) {
        ...
        if (idx < firstDay) {
          week.push({
            day: daysInPrevMonth - firstDay + idx + 1,
            year: y,
            month: m - 1,
            bgClass: 'bg-gray-200',
          ...
          week.push({
            ...
            year: y,
            month: m,
            bgClass: t ? 'bg-blue-100' : 'bg-gray-100',
            textClass: t ? 'text-blue-600 font-bold' : 'text-gray-800',
            events: ev,
          });
        } else {
          week.push({
  ...
  async function handleCreateEventClick() {
    ...
    if (imgInput && imgInput.files && imgInput.files.length > 0) {
      ...
      if (!jUpload.success)
        return alert('Image upload failed: ' + (jUpload.message || 'Server error'));
    ...
    if (!jEvent.success)
      return alert('Event creation failed: ' + (jEvent.message || 'Server error'));
